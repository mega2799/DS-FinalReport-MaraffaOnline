...
Rispettivamente si occupano di gestire la logica di gioco e la gestione degli utenti che vogliono utilizzare il sistema, due business core del progetto.

\subsection{Business Logic}

Si occupa di mantenere al proprio interno tutte le regole proprie del gioco e del calcolo dei punteggi. 
Ha svariati endpoint che permettono al middleware di avere il minor numero possibile di responsabilità, delegando al servizio di business logic le operazioni che riguardano il gioco.

Questa scelta è stata fatta per mantenere il middleware il più possibile leggero e per evitare che si occupi di operazioni che non gli competono. E per poter in un futuro aggiungere
nuovi giochi modificando il meno possibile il middleware che dovrà semplicemente occuparsi della gestione delle partite e comunicare tempestivamente con i servizi di business logic in 
base al gioco scelto.

\subsection{User Service}

Si occupa di gestire gli utenti, quindi la loro registrazione, autenticazione e gestione dei dati personali.
Anche in questo caso, il middleware si occupa di delegare al servizio di gestione degli utenti le operazioni che riguardano gli utenti, in modo da mantenere il middleware il più possibile leggero e per evitare che si occupi di operazioni che non gli competono.
Il core di questo servizio è dato dalla combinazione di NestJS e TypeORM, che permettono di gestire in modo semplice e veloce la persistenza dei dati e la gestione delle operazioni CRUD verso un database MySQL.
Che oltre alle informazioni degli utenti, contiene anche le informazioni riguardanti le partite giocate e statistiche personali di ogni utente del sistema.
